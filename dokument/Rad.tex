...
\section{Motivacija za odabir ZODB-a}
...
\begin{enumerate}
    \item \textbf{persistent.Persistent}
    \begin{itemize}
...
\section{Usporedba OODBMS vs RDBMS}
...
% **OVDJE UBACI SLIKU: uml_model_baze.png**
% Slika treba prikazivati Mermaid classDiagram sa ZODBRoot, Player, Enemy, Bullet, Item, GameDB klasama
\begin{figure}[H]
    \centering
    \textbf{[OVDJE UBACI SLIKU: uml\_model\_baze.png]}
    
    \vspace{1cm}
    \textit{Slika treba prikazivati UML dijagram klasa s klasama: ZODBRoot, Player, Enemy, Bullet, Item, GameDB i njihovim relacijama (agregacija, asocijacija). Player ima <<Persistent>> stereotip.}
    \vspace{1cm}
    
    % Kad budes imao sliku, odkomentiraj sljedeće:
...
% **OVDJE UBACI SLIKU: uml_aplikacija.png**
% Slika treba prikazivati GameApp, Menu, Button, InputBox klase i njihove relacije
\begin{figure}[H]
    \centering
    \textbf{[OVDJE UBACI SLIKU: uml\_aplikacija.png]}
    
    \vspace{1cm}
    \textit{Slika treba prikazivati UML dijagram klasa aplikacije: GameApp, Menu, Button, InputBox i njihove relacije. GameApp sadrži reference na GameDB, Player i Menu.}
    \vspace{1cm}
...
% **OVDJE UBACI SLIKU: dijagram_stanja.png**
% Slika treba prikazivati stateDiagram: MENU -> COUNTDOWN -> GAME -> GAME_OVER -> MENU
\begin{figure}[H]
    \centering
    \textbf{[OVDJE UBACI SLIKU: dijagram\_stanja.png]}
    
    \vspace{1cm}
    \textit{Slika treba prikazivati dijagram stanja aplikacije sa stanjima: MENU, COUNTDOWN, GAME, LOAD\_GAME, LEADERBOARD, GAME\_OVER. Prijelazi: MENU->COUNTDOWN (Start), COUNTDOWN->GAME, GAME->MENU (ESC), GAME->GAME\_OVER (HP=0), GAME\_OVER->MENU.}
    \vspace{1cm}
...
\hline
\texttt{name} & string & Jedinstveno ime igraca (ID). \\
\hline
...
\hline
\texttt{multiplier} & float & Mnozitelj bodova. \\
\hline
\texttt{time\_survived} & float & Vrijeme prezivljavanja u sekundama. \\
\hline
...
\begin{verbatim}
git clone https://github.com/yourusername/Projekt_ZODB_Igra.git
